...
// Check if email already exists in users or organizations table
try {
    $checkEmailUserQuery = "SELECT COUNT(*) as count FROM users WHERE email = :email";
    $checkEmailOrgQuery = "SELECT COUNT(*) as count FROM organizations WHERE email = :email";
    
    $checkStmt = $conn->prepare($checkEmailUserQuery);
